...
//Indices to build up a single vector for Ipopt
// state variables
size_t x_start = 0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332*Lf; // deg2rad(25) = 0.436332
  ...
  std::cout<< "Initial state values = " << state << std::endl;
  ...
  //steering and acceleration in only next time step
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);
  // green line - to see where the car will be in future
  for (int i=1; i<N-1; i++){
